...
| Landing CTAs → signup (LB-3) | Fake | landing CTAs (Sign In / Start Free / Begin Your Journey / Begin Free) still point to `/dashboard`, not `/auth/signup` (softened only by a middleware redirect to `/auth/login`) | Wrong acquisition destination (not yet addressed) | High |
...
**Bottom line:** the app is a **secure, live, functional MVP**. The original Phase 1 list (#1–#8) is done and #9 is blocked by the Free plan (Known Limitations). A later audit surfaced landing/pricing honesty issues: **LB-1** (fabricated stats + fictional testimonials) and **LB-2** (a fictional paid-tier pricing page with no billing behind it) are now both removed. **One sibling still gates launch — LB-3:** landing CTAs route to `/dashboard` instead of `/auth/signup`. Close that and the product surface is launch-ready. Phases 2 and 3 are post-launch retention/differentiation work.
